...
price_train_pred = pd.read_csv('Price_Train_pred.csv', header=None).as_matrix()[:50, :]
...
for hour in range(24):
	...
	for i in range(x_values.size):
		...
		total_cost = np.sum(result_bidding*my_bid_price*demand_train[:, hour]) + np.sum((1 - result_bidding)*7.*demand_train[:, hour])
		cost[i] = total_cost 
